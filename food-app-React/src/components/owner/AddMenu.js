...
export default function AddMenu() {
  ...
  const onSubmit=async(data,event)=>{
    ...
    alert("token received    "+OwnerJwtToken);
                    ...
                    <Grid container spacing={2}> 
                        ...
                        <Grid item xs={2}>
                          <TextField label="Item Id"   fullWidth  required
                           {...register(`itemId-${index}`,
                            {required:"Item ID is requied",
                              pattern: {
                                value: /^[0-9]{1,3}$/,
                                message: "Item id should be numeric with 1-3 digits"
                            }
                            })} 
                            error={!!errors[`itemId-${index}`]}  // Dynamically access the error
                            helperText={errors[`itemId-${index}`] ? errors[`itemId-${index}`].message : ""}  // Dynamically show error message
